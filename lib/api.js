...
export default {
	...
	async createServices(service) {
		...
		if (!response.ok)
			throw new Error('Ocurrió un error al crear los servicios');
	...
	async createQuotation(quotation) {
		...
		if (!response.ok)
			throw new Error('Ocurrió un error al crear las cotizaciones');
	...
	async getEvent() {
		...
		const { data } = jsonBody;
		if (!response.ok)
			throw new Error('Ocurrió un error al obtener los eventos');
	...
	async getNeedById(id) {
		...
		if (!response.ok)
			throw new Error('Ocurrió un error al obtener la necesidad');

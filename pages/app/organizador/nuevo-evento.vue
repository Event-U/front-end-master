<template>
  <div>
    <div class="event-preview mt-3" v-if="currentStepNumber >= 5 && formInProgress">
      <h1 class>¡Éste es tu nuevo evento!</h1>
    ...
    <div class="needs-preview row m-0 mt-3" v-if="currentStepNumber === 4">
      <div v-for="(need, i) in eventObject.needs" class="col-md-3 need-col" :key="i">
        <event-need v-bind="need" />
      </div>
    </div>
    ...
    <div
      v-if="this.asyncState === 'pending' && !formInProgress"
    ...
    <div class="event-created text-center" v-if="this.asyncState === 'success' && !formInProgress">
      <h1>¡Felicidades, creaste tu evento!</h1>
      <nuxt-link
        class="btn btn-iconed animated fadeInUp mt-3"
        :to="`/app/planificador/tablero/evento/${newEventId}`"
      >Planificar mi evento</nuxt-link>
    </div>
  </div>
</template>
...
export default {
  ...
  components: {
    ...
    EventBasicInfo: () =>
      import(
        /* webpackChunkName: "EventBasicInfo" */ '@/components/ui/forms/EventBasicInfo.vue'
      ),
    EventSpecificInfo: () =>
      import(
        /* webpackChunkName: "EventSpecificInfo" */ '@/components/ui/forms/EventSpecificInfo.vue'
      ),
    EventCard: () =>
      import(
        /* webpackChunkName: "EventCard" */ '@/components/ui/cards/EventCard.vue'
      ),
    EventNeed: () =>
      import(
        /* webpackChunkName: "EventNeed" */ '@/components/ui/cards/EventNeed.vue'
      ),
    NewEventAdress: () =>
      import(
        /* webpackChunkName: "NewEventAdress" */ '@/components/sections/forms/NewEventAdress.vue'
      ),
  },
  ...
  computed: {
    ...
    ...mapState({
      ...
      newEventId: (state) => state.events.newEvent._id,
    }),
  },
  methods: {
    update(e) {
    ...
    addNewAdress(e) {
      this.eventObject.address = this.$store.state.addresses.newAdress._id;
      this.canGoNext = true;
    },
